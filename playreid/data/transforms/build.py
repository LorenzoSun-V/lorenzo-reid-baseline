...
def build_transforms(cfg, is_train=True):
    ...
    if is_train:
        ...
        if size_train[0] > 0:
            res.append(T.Resize(size_train[0] if len(size_train) == 1 else size_train, interpolation=T.InterpolationMode.BICUBIC))
        ...
        res.append(T.Normalize(mean=cfg.MODEL.PIXEL_MEAN, std=cfg.MODEL.PIXEL_STD))
    else:
        ...
        if size_test[0] > 0:
            res.append(T.Resize(size_test[0] if len(size_test) == 1 else size_test, interpolation=T.InterpolationMode.BICUBIC))
        if do_crop:

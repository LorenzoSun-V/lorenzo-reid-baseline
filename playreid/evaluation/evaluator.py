...
from playreid.utils.logger import log_every_n_seconds
...
def inference_on_dataset(model, data_loader, evaluator, flip_test=False):
    ...
    with inference_context(model), torch.no_grad():
        for idx, inputs in enumerate(data_loader):
            # if '/workspace/lorenzo/ReID/datasets/cmdm/market1501/gallery/0102/0102_c4s1_017301_03.jpg' in inputs['img_paths']:
            #     the_index = inputs['img_paths'].index('/workspace/lorenzo/ReID/datasets/cmdm/market1501/gallery/0102/0102_c4s1_017301_03.jpg')
            #     print(f"{inputs['img_paths'][the_index]}: {inputs['images'][the_index]}")
            #     outputs = model(inputs, the_index)
            #     exit(0)
            if idx == num_warmup:
                start_time = time.perf_counter()
                total_compute_time = 0
            ...
            outputs = model(inputs['images'])[..., 0, 0]
            # Flip test
            if flip_test:
                inputs["images"] = inputs["images"].flip(dims=[3])
                flip_outputs = model(inputs)[..., 0, 0]
